...
call_eg_function_with_timeout() {
	...
	printf "UNTIMED CALL!,github_repo_name=$github_repo_name,github_branch_name=$github_branch_name,gitlab_commit_sha=$gitlab_commit_sha"
	#export TMP_GITLAB_BUILD_STATUS_FILEPATH=$TMP_GITLAB_BUILD_STATUS_FILEPATH
	#timeout 120s bash -c "source src/import.sh && manage_get_gitlab_ci_build_status $github_repo_name $github_branch_name $gitlab_commit_sha"
	manage_get_gitlab_ci_build_status $github_repo_name $github_branch_name $gitlab_commit_sha
	printf "DONE TIMED CALL"
	...
	printf "AFTER EXPORTING AND IMPORTING, IN call_eg_function_with_timeout read_status=$read_status"
	echo "$read_status"
}